...
class ProjectionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.proj(x) # self.softmax(self.proj(x))
...
# generally the d_ff is the 4 times of d_model
# so, if i want to further decrease the dimension of d_model as 160, then, the d_ff = 160 * 4 = 640   
def build_transformer(src_vocab_size: int,  src_seq_len: int, d_model: int=160, N: int=6, h: int=8, dropout: float=0.0, d_ff: int=640) -> Transformer:
